...
const cwd = process.cwd();
const copy = (src, dest) => {
  const state = require$$0$1.statSync(src);
  require$$0$1.cpSync(src, dest, { recursive: state.isDirectory() });
};
const createProject = async () => {
  try {
    const result = await prompts$1([
      {
        type: "text",
        name: "project",
    ...
    const templateDir = require$$0$2.resolve(__dirname, `../src/templates/${result.template}`);
    copy(templateDir, targetDir);
    const packageJsonPath = `${targetDir}/package.json`;
    const packageJson = JSON.parse(require$$0$1.readFileSync(packageJsonPath, "utf-8"));
    ...
    require$$0$1.writeFileSync(packageJsonPath, JSON.stringify(packageJson, null, 2));
